...
SWISH_GAMES <- list.files(path = "./Desktop/Heat Check/2012-13", pattern = "*NYK+") %>% c(list.files(path = "./Desktop/Heat Check/2013-14", pattern = "*NYK+"))
SWAGGYP_GAMES <- list.files(path = "./Desktop/Heat Check/2012-13", pattern = "*PHI+") %>% c(list.files(path = "./Desktop/Heat Check/2013-14", pattern = "*LAL+"))
...
for (gm in 1:length(SWISH_GAMES)) {
  file <- paste("~/Desktop/Heat Check/2012-13/", SWISH_GAMES[gm], sep = "")
  if (!file.exists(file)) file <- paste("~/Desktop/Heat Check/2013-14/", SWISH_GAMES[gm], sep = "")
...
for (gm in 1:length(SWAGGYP_GAMES)) {
  file <- paste("~/Desktop/Heat Check/2012-13/", SWAGGYP_GAMES[gm], sep = "")
  if (!file.exists(file)) file <- paste("~/Desktop/Heat Check/2013-14/", SWAGGYP_GAMES[gm], sep = "")
  game <- read_csv(file, col_types = cols())
